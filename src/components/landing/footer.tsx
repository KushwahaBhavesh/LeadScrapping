                ...
                <div className="flex flex-col lg:flex-row justify-between items-start gap-12 md:gap-16 pb-20 md:pb-32 border-b border-white/5">
                    <div className="space-y-8 md:space-y-12 max-w-2xl">
                        ...
                        <p className="text-2xl md:text-3xl lg:text-4xl font-black text-foreground/30 leading-[1.1] tracking-tighter italic uppercase max-w-xl">
                            The definitive <span className="text-foreground">Data Infrastructure</span> for high-velocity revenue teams.
                        </p>
                    </div>

                    <div className="flex flex-col items-start lg:items-end gap-8 md:gap-12">
                        <div className="flex flex-col items-start lg:items-end gap-2">
                            <div className="text-[9px] md:text-[10px] font-mono font-black uppercase tracking-[0.5em] text-primary/40">CORE_NETWORK_UPLINK</div>
                            <div className="text-[11px] md:text-[12px] font-mono font-black uppercase tracking-[0.2em] text-foreground/20">STATUS: 1,482 NODES_ACTIVE</div>
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-16 py-16 md:py-24">
                    {footerLinks.map((col, i) => (
                        <div key={i} className="space-y-6 md:space-y-10">
                            <div className="flex items-center gap-4">
                                ...
                                <h4 className="text-[10px] md:text-[11px] font-mono font-black uppercase tracking-[0.4em] text-foreground/20">{col.title}</h4>
                            </div>
                            <ul className="space-y-4 md:space-y-5">
                                {col.links.map((link, j) => (
                                    <li key={j}>
                                        <Link href="#" className="group flex justify-between items-center text-foreground/40 hover:text-foreground font-black text-[13px] md:text-[15px] uppercase tracking-tighter transition-all italic">
                                            <span>{link}</span>
                    ...
                    <div className="lg:border-l border-white/5 lg:pl-16 space-y-6 md:space-y-10">
                        <div className="flex items-center gap-4">
                            ...
                            <h4 className="text-[10px] md:text-[11px] font-mono font-black uppercase tracking-[0.4em] text-foreground/20">SYSTEM_METRICS</h4>
                        </div>
                        <div className="space-y-3 md:space-y-4">
                            {[
                                ...
                                <div key={i} className="flex items-center justify-between p-3 md:p-4 rounded-xl md:rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group cursor-default">
                                    <div className="flex items-center gap-3">
                                        <stat.icon className={cn("h-3.5 w-3.5 md:h-4 md:w-4", `text-${stat.color}`)} />
                                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-foreground/20 group-hover:text-foreground/40 transition-colors">{stat.label}</span>
                                    </div>
                                    <span className="text-[9px] md:text-[10px] font-mono font-black text-primary/60">{stat.val}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="pt-12 md:pt-16 border-t border-white/5 flex flex-col lg:flex-row justify-between items-center gap-8 md:gap-10">
                    ...
                    <div className="flex items-center gap-5 md:gap-8 px-4 md:px-6 py-3 rounded-xl md:rounded-2xl bg-white/[0.02] border border-white/5">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-[8px] md:text-[9px] font-mono font-black text-foreground/20 tracking-[0.2em] uppercase">Architecture_v5.29.X // SECURE_ACCESS</div>
                        </div>
