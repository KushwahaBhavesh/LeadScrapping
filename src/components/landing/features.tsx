...
const features = [
  {
    ...
    description:
      'Intelligent selectors that adapt to website layout changes automatically. Maintain zero-downtime extractors.',
    icon: Zap,
  ...
  },
  {
    ...
    description:
      'Access local data from 190+ countries with our residential proxy network. Bypass geo-blocks instantly.',
    icon: Globe,
  ...
  },
  {
    ...
    description:
      'Multi-step verification process to ensure every email and phone number is valid and active before export.',
    icon: Shield,
    className: 'bg-background border-white/10',
    accent: 'REAL_DATA',
    metrics: '< 1% Bounce',
  },
  {
    ...
    description:
      'Push verified leads directly to HubSpot, Salesforce, or Pipedrive. Automate your entire outbound workflow.',
    icon: ChartBar,
...
export function Features() {
  ...
  return (
    <section
      id="features"
      ref={containerRef}
      className="py-24 md:py-48 px-4 relative bg-background overflow-hidden grain"
    >
        ...
        <div className="mt-24 flex flex-col items-center space-y-8">
          ...
          <Magnetic>
            <Link href="/register">
              <Button
                size="lg"
                className="h-18 px-14 text-lg font-black rounded-2xl bg-primary text-white hover:scale-[1.05] transition-all shadow-[0_20px_50px_rgba(var(--color-primary),0.3)] group uppercase tracking-widest active:scale-95"
              >
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-6 right-6 flex justify-between items-center pointer-events-none">
        <div className="font-mono text-[8px] text-foreground/20 uppercase tracking-widest">
          {feature.accent}
        </div>
        <div className="flex gap-1">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-1 w-1 rounded-full bg-primary/20 group-hover:bg-primary/50 transition-colors"
            />
          ))}
        </div>
      </div>
