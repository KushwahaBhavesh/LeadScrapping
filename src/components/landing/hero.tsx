    ...
    >
      ...
      <div className="container px-4 mx-auto relative z-10">
        <div className="flex flex-col items-center text-center space-y-10">
          ...
          <div className="space-y-5 max-w-4xl">
            <h1 className="text-display-xl font-black tracking-tight text-foreground leading-[1.05] text-glow">
              {['Automate', 'Lead', 'Extraction', 'at'].map((word, i) => (
                <motion.span
                  key={i}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            <motion.p
              ...
              className="text-lg md:text-2xl text-foreground/50 max-w-3xl mx-auto font-medium leading-relaxed"
            >
              Deploy AI-powered scrapers to find, verify, and export business leads instantly.
              Bypass anti-bot protections and access high-fidelity data from any source.
            </motion.p>
          </div>
          ...
          >
            ...
            <Magnetic>
              <Button
                size="lg"
                variant="outline"
                className="w-full sm:min-w-[200px] h-14 text-base font-bold rounded-full border-border bg-background text-foreground hover:bg-muted transition-all active:scale-95"
              >
                View Live Demo
              </Button>
            </Magnetic>
          ...
          >
            <div className="flex flex-col items-center gap-10">
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-foreground/20">
                Powering the extraction layers for
              </span>
              <Marquee duration={25}>
                {[
        ...
        <motion.div
          ...
          className="mt-32 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border border-border rounded-[32px] overflow-hidden bg-background/50 backdrop-blur-xl shadow-2xl"
        >
          {[
            {
              title: '500M+ Records',
              desc: 'Live index of global B2B profiles, firmographics.',
              icon: Database,
            },
            {
              title: 'Agentic Validation',
              desc: 'AI nodes verify leads in real-time.',
              icon: Shield,
            },
            {
              title: 'CRM Sync',
              desc: 'Native integrations with HubSpot/Salesforce.',
              icon: Sparkles,
            },
          ].map((item, i) => (
            <div
              key={i}
              className={cn(
                'p-10 transition-all hover:bg-muted/50 flex flex-col space-y-5 group cursor-default backdrop-blur-sm',
                i < 2 ? 'md:border-r border-border' : '',
                i < 2 ? 'border-b md:border-b-0 border-border' : ''
              )}
            >
              <div className="h-9 w-9 rounded-lg bg-primary flex items-center justify-center group-hover:rotate-12 transition-transform duration-500 shadow-md">
                <item.icon className="h-4.5 w-4.5 text-primary-foreground" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl md:text-3xl font-black tracking-tight text-foreground group-hover:text-primary transition-colors uppercase ">
                  {item.title}
                </h3>
                <p className="text-base md:text-lg text-foreground/50 font-medium leading-relaxed  tracking-tight">
                  {item.desc}
                </p>
              </div>
            </div>
